<template>

    <div class="animate__animated animate__fadeInDown"
         style="font-family: Iceland, sans-serif; font-size: 40px; color: #d3d3d3; margin-left: 60%; word-spacing: 1em; position: fixed; z-index: 20">
        <router-link to="#"> assortiment</router-link>
        <router-link to="#"> basket</router-link>
        <router-link to="#"> support</router-link>
    </div>

    <div class="leftinfo header"
         style="margin-left: 100%; margin-top: 9%; position: absolute; z-index: 6; width: 50%; height: 5%">
        <div class="mt-5 title__film stroke"
             style="font-family: Iceland, sans-serif; color: white; font-size: 50px; margin-left: 20%">
            Film for your cars
        </div>
        <div class="subtitle__film stroke"
             style="font-family: Iceland, sans-serif; color: white; font-size: 30px; line-height: 30px; margin-left: 20%">
            <p> The ideal coating of any color and size,</p>
            <p>order or look for it in car dealerships in your city</p>
        </div>
    </div>



    <v-img style="position: absolute; z-index: 1; margin-left: 4%" class="animate__animated animate__flipInY animate__two mt-6" src="./src/assets/logo.svg" width="200"/>



    <div class="svg-attributes-demo polygon"></div>

    <svg class="title_paralax" style="margin-left: 10%; margin-top: 8%; z-index: 1; position: absolute" width="1165" height="414"
         viewBox="0 0 1165 414" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path class="popa"
              d="M519.082 203.008L585.303 203.008M519.082 203.008L509.582 48.499L486.798 183.453L441.082 183.453L412.582 58.9531L407.082 203.008L294.082 203.008M519.082 203.008L415.082 234.453M519.082 203.008L580.582 222.953L580.582 263.953L544.082 263.953L534.082 241.453L504.082 245.453L492.082 310.953L504.082 367.818L530.082 367.818L534.082 330.453L512.582 324.953L512.582 288.953L589.082 288.953L589.082 407.47M519.082 203.008L473.582 216.765L432.082 263.953L429.082 330.453L449.582 383.953L486.798 407.472L524.082 407.47L557.582 389.953L557.582 407.47L589.082 407.47M179.173 201.501L179.183 2.50771L119.706 2.52433L119.706 60.4956L129.706 128.995L73.1639 2.49456L4.17649 2.49352L15.1761 21.9935L15.2561 201.497L73.164 203.008L73.1639 142.453L62.0821 79.999L125.582 203.008M179.173 201.501L231.702 20.5308L226.208 2.52433L302.224 2.52276L353.227 192.544L367.225 2.5971L451.195 2.57561L469.294 96.5851L486.798 2.49256L561.292 2.55878L575.829 203.008L585.303 203.008M179.173 201.501L125.582 203.008M179.173 201.501L137.582 208.5L105.082 241.453L96.582 278.953L96.582 343.453L110.082 383.953L137.582 402.999L171.082 411.953L213.082 407.47L241.582 389.953L255.082 362.453M585.303 203.008L585.303 2.51351L706.296 2.46388L703.294 48.499L642.795 44.499L642.795 79.999L690.295 79.999L690.295 122.999L642.795 118.499L642.795 159.999L708.309 158.001L708.338 208.5M585.303 203.008L708.338 208.5M708.338 208.5L764.833 402.999L764.835 227.487L754.339 205.983L824.349 205.977L880.998 330.976L870.999 260.476L870.999 205.983L929.478 205.978L929.421 407.471M708.338 208.5L633.582 205.966L637.582 222.953L589.082 407.47M929.421 407.471L946.901 384.469M929.421 407.471L875.082 407.47L813.582 285.453L824.349 343.453L824.349 407.47L699.082 407.47L667.582 267.953L655.082 336.453L683.332 337.711L692.079 376.453L647.082 376.453L642.795 407.471L589.082 407.47M946.901 384.469L946.884 205.975L1009.38 205.966L1009.42 407.471L1022.47 407.471L1022.45 368.453L1079.52 260.481L1095.03 247.961L1028.53 252.961L1028.53 205.966L1162.04 205.961L1162.04 238.454L1095.03 355.954L1085.54 364.454L1162.07 360.954L1162.08 407.47L946.884 407.47L946.901 384.469ZM294.082 203.008L260.582 64.4531L248.582 133.731L277.332 133.731L294.082 203.008ZM294.082 203.008L286.694 172.453L241.582 172.453M294.082 203.008L273.582 205.966L273.582 407.471L334.082 407.47L334.082 343.453L353.227 343.453L362.082 398.953L366.082 407.47L432.082 407.47L421.446 383.953L418.082 371.953L407.082 337.711L393.582 330.453L418.082 304.453L421.446 274.453L415.082 234.453M241.582 172.453L236.582 203.008L125.582 203.008M241.582 172.453L236.829 201.497L181.082 203.008L227.082 214.453L241.582 227.453L255.082 257.953L261.082 288.953M255.082 362.453L261.082 324.953L261.082 288.953M255.082 362.453L192.582 367.818L196.582 294.453L192.582 245.453L179.173 234.453L160.582 248.953L164.082 362.453L185.082 376.453L261.082 288.953M415.082 234.453L389.582 208.5L277.332 208.5L334.082 241.453M415.082 234.453L334.082 241.453M334.082 241.453L334.082 304.453L358.582 294.453L358.582 248.953L334.082 241.453Z"
              stroke="white" stroke-width="4"/>
    </svg>


    <div>
        <v-img class="header_img" src="./src/assets/car.png" width="1000" height="500"
               style=" margin-left: 25%; margin-top: 25%; z-index: 2; position: absolute"/>
        <v-img class="header_img" src="./src/assets/carpurple.png" width="1000" height="500"
               style=" margin-left: 24%; margin-top: 25%; z-index: 3; position: absolute"/>
        <div class="neons"
             style="margin-top: 37%; margin-left: 45%; width: 20px; height: 20px; z-index: 5; position: absolute"/>
        <v-img class="header_imghells" src="./src/assets/1.png" width="160" height="173" id="whellItems"
               style="position: absolute; z-index: 4; margin-top: 41%; margin-left: 49%"/>
        <v-img class="header_imghells" src="./src/assets/1.1.png" width="157" height="150" id="whellItemsTwo"
               style="position: absolute; z-index: 4; margin-top: 41%; margin-left: 72%"/>
    </div>


    <v-img class="animate__infinite animate__animated animate__fadeOutDown" src="./src/assets/lineONE.png"
           width="65" height="50" style="z-index: 3; position: absolute; margin-top: 45%; margin-left: 10%"/>
    <v-img class="animate__infinite animate__animated animate__fadeOutDown" src="../src/assets/lineTWO.png"
           width="80" height="50" style="z-index: 3; position: absolute; margin-top: 43%; margin-left: 9.5%"/>


    <div class="d-flex section-title" style="background-color: black; width: fit-content; height: 8vh; color: #4f4f4f; font-family: Iceland, sans-serif; font-size: 40px; margin-top: 54%">

      <InfoTop/>

    </div>


</template>


<script setup lang="ts">

import anime from 'animejs/lib/anime.es.js';
import {onMounted, ref} from "vue";
import {gsap} from "gsap";
import {ScrollTrigger} from "gsap/ScrollTrigger";
import InfoTop from "./InfoTop.vue";

gsap.registerPlugin(ScrollTrigger);

const imgItem = ref<HTMLElement>({} as HTMLElement)
let currentWheel = 0

const wheelCar = () => {
    imgItem.value = document.querySelector('#whellItems img')
    switch (currentWheel) {
        case 0:
            imgItem.value.setAttribute("src", "/src/assets/1.png");
            break
        case 1:
            imgItem.value.setAttribute("src", "/src/assets/2.png");
            break
        case 2:
            imgItem.value.setAttribute("src", "/src/assets/3.png");
            break
        case 3:
            imgItem.value.setAttribute("src", "/src/assets/4.png");
            break
        case 4:
            imgItem.value.setAttribute("src", "/src/assets/5.png");
            break
        case 5:
            imgItem.value.setAttribute("src", "/src/assets/7.png");
            break
        case 6:
            imgItem.value.setAttribute("src", "/src/assets/8.png");
            break
        case 7:
            imgItem.value.setAttribute("src", "/src/assets/9.png");
            break
        case 8:
            imgItem.value.setAttribute("src", "/src/assets/10.png");
            break
        case 9:
            imgItem.value.setAttribute("src", "/src/assets/11.png");
            break
    }

    currentWheel++
    if (currentWheel >= 10) {
        currentWheel = 0
    }
}

const wheelCarTwo = () => {
    imgItem.value = document.querySelector('#whellItemsTwo img')
    switch (currentWheel) {
        case 0:
            imgItem.value.setAttribute("src", "/src/assets/1.1.png");
            break
        case 1:
            imgItem.value.setAttribute("src", "/src/assets/2.1.png");
            break
        case 2:
            imgItem.value.setAttribute("src", "/src/assets/2.3.png");
            break
        case 3:
            imgItem.value.setAttribute("src", "/src/assets/2.4.png");
            break
        case 4:
            imgItem.value.setAttribute("src", "/src/assets/2.5.png");
            break
        case 5:
            imgItem.value.setAttribute("src", "/src/assets/2.6.png");
            break
        case 6:
            imgItem.value.setAttribute("src", "/src/assets/2.7.png");
            break
        case 7:
            imgItem.value.setAttribute("src", "/src/assets/2.8.png");
            break
        case 8:
            imgItem.value.setAttribute("src", "/src/assets/2.9.png");
            break
        case 9:
            imgItem.value.setAttribute("src", "/src/assets/10.1.png");
            break
    }

    currentWheel++
    if (currentWheel >= 10) {
        currentWheel = 0
    }
}

onMounted(() => {

    setInterval(() => {
        wheelCar()
    }, 50)
    setInterval(() => {
        wheelCarTwo()
    }, 50)

    setTimeout(() => {
        anime({
            targets: '.iconanimat',
            translateX: 270,
            direction: 'alternate',
            loop: 1,
            duration: 1000,
            easing: function (el, i, total) {
                return function (t) {
                    return Math.pow(Math.sin(t * (i + 1)), total);
                }
            }
        })
    }, 2000)

    setTimeout(() => {
        anime({
            targets: '.leftinfo',
            translateX: -800,
            direction: 'alternate',
            easing: 'easeInOutSine',
            endDelay: 6000,
        });
    }, 60)


    setTimeout(() => {
        gsap.to('.title__film', {
            x: 150,
            scale: 1.5,
            duration: 2,
        })
    }, 1000)

    setTimeout(() => {
        gsap.to('.subtitle__film', {
            x: -150,
            scale: 0.8,
            duration: 2,
        })
    }, 1000)


    gsap.to('.title_paralax', {
        scrollTrigger: {
            trigger: '.header',
            start: 'top top',
            scrub: 1.9
        },
        yPercent: -150
    })

    gsap.to('.header .stroke', {
    scrollTrigger: {
        trigger: 'header',
        start: 'top top',
        scrub: 1.9
    },
    xPercent: 500
    })

    gsap.to('.header_img', {
        scrollTrigger: {
            trigger: 'header',
            start: 'top top',
            scrub: 1.9
        },
        xPercent: -700
    })

    gsap.to('.header_imghells', {
        scrollTrigger: {
            trigger: 'header',
            start: 'top top',
            scrub: 1.9
        },
        xPercent: -4470
    })

    gsap.to('.section-title', {
        scrollTrigger: {
            trigger: 'header',
            start: 'top top',
            scrub: 1.9
        },
        xPercent: -20,
    })

    setTimeout(() => {
        livLogo()
    }, 1500)



})



const livLogo = () => {
    document.querySelector('.animate__two').classList.add('animate__flipOutY')
}



</script>


<style scoped>


@keyframes aw {
    0% {

    }
    30% {
        rotate: 90deg;
    }
    60% {
        rotate: 180deg;
    }
    100% {
        rotate: 360deg;
    }
}

.neons {
    animation: neon .3s alternate-reverse infinite;
    animation-iteration-count: 5;
}

@keyframes neon {
    from {
        background-color: rgba(255, 255, 255, 1);
        width: 2px;
        height: 2px;
        filter: blur(5px);
    }
    to {
        background-color: rgba(255, 255, 255, 1);
        width: 20px;
        height: 20px;
        filter: blur(10px);
    }

}

.popa {
    stroke-dasharray: 10000;
    stroke-dashoffset: 10000;
    animation: pop 15s linear forwards;
}

@keyframes pop {
    to {
        stroke-dashoffset: 0;
    }
}


</style>
